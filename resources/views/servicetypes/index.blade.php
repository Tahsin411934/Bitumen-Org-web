<x-app-layout>
    <div class="mx-auto w-[98%] ">
        <div class="flex justify-between items-center w-[96%] mx-auto my-8">
            <h1 class="text-2xl font-bold">Service Type </h1>
        ...
        <div class="bg-white shadow-lg rounded-lg p-8 w-full overflow-x-auto">
            ...
            <table id="example" class="display w-full">
                ...
                <tbody>
                    ...
                    <tr>
                        <form action="{{ route('service-type.update', $serviceType->id) }}" method="POST" class="update-form">
                            @csrf
                            @method('PUT')
                            <td>
                                <input type="text" name="id" value="{{ $serviceType->id }}" 
                                    class="w-full border border-gray-300 rounded px-2 py-1" disabled />
                            </td>
                            <td>
                                <input type="text" name="servicename" value="{{ $serviceType->servicename }}" 
                                    class="w-full border border-gray-300 rounded px-2 py-1" disabled />
                            </td>
                            ...
                            <form action="{{ route('service-type.destroy', $serviceType->id) }}" method="POST" class="inline">
                                @csrf
                                @method('DELETE')
        ...
        <div class="relative w-full max-w-2xl mx-auto p-4">
            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                ...
                <div class="flex justify-between items-center p-4 md:p-5 border-b rounded-t dark:border-gray-600">
                    <h3 class="text-xl font-semibold text-gray-900 dark:text-white">Add Service Type</h3>
                    <button type="button"
                ...
                <!-- Modal Body -->
                <form action="{{ route('service-type.store') }}" method="POST" class="p-6 space-y-6">
                    @csrf
                    <div class="grid  gap-6">
                        <input type="text" name="servicename" 
                            class="p-3 border border-gray-300 rounded-lg shadow-sm w-full" placeholder="Service Name">
                    </div>
