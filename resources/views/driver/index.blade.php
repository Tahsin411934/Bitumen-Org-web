        ...
        <div class="bg-white shadow-lg rounded-lg p-8 w-full overflow-x-auto">
            ...
            <table id="example" class="display w-full">
                ...
                <tbody>
                    ...
                    <tr>
                        <form action="{{ route('drivers.update', $driver->driver_id) }}" method="POST" class="update-form">
                            @csrf
                            @method('PUT')
                            <td>{{ $driver->driver_id }}</td>
                            <td>
                                <textarea name="name" class="w-full border border-gray-300 rounded px-2 py-1 resize-none" disabled>{{ $driver->name }}</textarea>
                            </td>
                            <td>
                                <textarea name="license_no" class="w-full border border-gray-300 rounded px-2 py-1 resize-none" disabled>{{ $driver->license_no }}</textarea>
                            </td>
                            <td>
                                <textarea name="nid_no" class="w-full border border-gray-300 rounded px-2 py-1 resize-none" disabled>{{ $driver->nid_no }}</textarea>
                            </td>
                            <td>
                                <textarea name="contact_no" class="w-full border border-gray-300 rounded px-2 py-1 resize-none" disabled>{{ $driver->contact_no }}</textarea>
                            </td>
                            <td>
                                <textarea name="truck_id" class="w-full border border-gray-300 rounded px-2 py-1 resize-none" disabled>{{ $driver->truck_id }}</textarea>
                            </td>
    ...
    <div id="default-modal" tabindex="-1" aria-hidden="true"
        ...
        <div class="relative w-full max-w-2xl mx-auto p-4">
            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                ...
                <!-- Modal Body -->
                <form action="{{ route('drivers.store') }}" method="POST" class="p-6 space-y-6">
                    @csrf
                    <div class="grid grid-cols-2 gap-6">
                        <input type="text" name="name" required
                            class="p-3 border border-gray-300 rounded-lg shadow-sm w-full" placeholder="Driver Name">
                        <input type="text" name="license_no" 
                            class="p-3 border border-gray-300 rounded-lg shadow-sm w-full" placeholder="License No">
                    </div>
                    <div class="grid grid-cols-2 gap-6">
                        <input type="text" name="nid_no" 
                            class="p-3 border border-gray-300 rounded-lg shadow-sm w-full" placeholder="NID No">
                        <input type="text" name="contact_no" 
                            class="p-3 border border-gray-300 rounded-lg shadow-sm w-full" placeholder="Contact No">
                    </div>
                    <div class="grid grid-cols-2 gap-6">
